...
##### Add the cost of sequestering the non-CO2s embodied in each liter of synfuel
#Each column has the cost of sequestering cabron from 50 $/ton CO2 to 350 $/ton CO2
# Each row has the mamount of non-CO2s in kg based on different GWP, thus multipliers:
      #GWP100 multiplier 1.7
      #GWP50 multiplier 2.3
      #GWP20 multiplier 4.0

synthetic_fuel_CDR_cost <- read_csv("synthetic_fuel_CDR_cost.csv")

synthetic_fuel_CDR_cost %>%
  data.matrix()-> synthetic_fuel_CDR_cost

#Create x & y arrays
CDR_cost <- seq(0, 350, by = 50) #Hydrogen cost in $/kg H2 (x axis)
Multiplier_syn <- seq(1.5, 5, by = 0.5) #Carbon cost in %/kg CO2 (y axis)


fig_synfuels_liter_CDR <- plot_ly(x = CDR_cost, #This is the column in matrix
                              y = Multiplier_syn, #This is the row in matrix
                              z = synthetic_fuel_CDR_cost,
                              type = "contour",
                              colors = colors_figures_2,
                              colorbar = list(bordercolor = "pink",
  ...
  layout(title = "Synthetic Fuels (with CDR)",
         xaxis = list(
...
print(fig_synfuels_liter_CDR)


# Save the figure as both PNG and vector file to modify in Illustrator
orca(fig_synfuels_liter_CDR, "fig4_synthetic_fuels_liter_CDR.png")
orca(fig_synfuels_liter_CDR, "fig4_synthetic_fuels_liter_CDR.svg")
...
##### Add the cost of sequestering the non-CO2s embodied in each liter of synfuel
#Each column has the cost of sequestering carbon from 50 $/ton CO2 to 350 $/ton CO2
# Each row has the amount of non-CO2s in kg based on different GWP, thus multipliers:
      #GWP100 multiplier 1.7
      #GWP50 multiplier 2.3
      #GWP20 multiplier 4.0

HEFA_fuel_CDR_cost <- read_csv("HEFA_fuel_CDR_cost.csv")

HEFA_fuel_CDR_cost %>%
  data.matrix()-> HEFA_fuel_CDR_cost

#Multipliers for biofuels since there are more values in y axis
Multiplier_bio <- seq(1.5, 5, by = 0.39) #Carbon cost in %/kg CO2 (y axis)

fig_HEFA_liter_CDR <- plot_ly(x = CDR_cost, #This is the column in matrix
                                  y = Multiplier_bio, #This is the row in matrix
                                  z = HEFA_fuel_CDR_cost,
                                  type = "contour",
                                  colors = colors_figures_2,
                                  colorbar = list(bordercolor = "pink",
                                                  title = "Cost ($/liter)",
...
print(fig_HEFA_liter_CDR)


# Save the figure as both PNG and vector file to modify in Illustrator
orca(fig_HEFA_liter_CDR, "fig4_HEFA_fuels_liter_CDR.png")
orca(fig_HEFA_liter_CDR, "fig4_HEFA_fuels_liter_CDR.svg")
...
FT_fuel_CDR_cost <- read_csv("FT_fuel_CDR_cost.csv")

FT_fuel_CDR_cost %>%
  data.matrix()-> FT_fuel_CDR_cost

fig_FT_CDR <- plot_ly(x = CDR_cost, #This is the column in matrix
                  y = Multiplier_bio, #This is the row in matrix
                  z = FT_fuel_CDR_cost,
                  type = "contour",
                  colors = colors_figures_2,
...
# Save the figure as both PNG and vector file to modify in Illustrator
orca(fig_FT_CDR, "fig4_FT_L_CDR.png")
orca(fig_FT_CDR, "fig4_FT_L_CDR.svg")
...
carbon_seq_cost <-  seq(0, 350, by = 50) #Carbon sequestration cost in USD per ton of CO2

# Create a blank matrix
z4_cost <- matrix(,nrow = length(carbon_seq_quant), ncol = length(carbon_seq_cost))

# Iterate through arrays to populate matrix
for (i in 1:length(carbon_seq_quant)){
  for (j in 1:length(carbon_seq_cost)){
    z4_cost[i, j] <- ((carbon_seq_quant[i] * carbon_seq_cost[j])/10^3) #result will be in trillion USD
  }

}

# contour(z)
fig_co2_seq_cost <- plot_ly(x = carbon_seq_cost, # This represents columns
                        y = carbon_seq_quant, # This represents rows
                        z = z4_cost,
                        type = "contour",
                        colors = colors_figures_2,
                        colorbar = list(
...
#==============================================================================================================
# Now we want to estimate what is cheaper? To mitigate emissions by passenger-kilometer-equivaler / ton-kilometer-equivalent using SAF or using CDR?
